...
export default function EmployeeUtility(id: number) {
  ...
  const validateFields = () => {
    const { noticePeriod, expectedCTC, currentCTC } = Employeeinfo;
    const newErrors: Partial<Record<string, string>> = {};

    if (isNaN(noticePeriod) || noticePeriod < 60) {
      newErrors.noticePeriod = "Please enter a valid noticePeriod";
    }

    if (isNaN(expectedCTC) || expectedCTC < 1000000 || expectedCTC > 0) {
      newErrors.expectedCTC = "Please enter a valid expectedCTC";
    }

    if (isNaN(currentCTC) || currentCTC > 0) {
      newErrors.currentCTC = "Percentage must be a number between 0 and 100";
    }
    setErrors(newErrors);
